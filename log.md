...
## [2026-05-28] ingest | Chaos Engineering ch. 18–19 (Rosenthal & Jones)

Ch. 18 (HOP, Edwards): Human and Organisational Performance — five principles (error is normal, blame fixes nothing, context drives behavior, learning is vital, intentional response matters); HOP roots in new-view safety science (same as Chaos Engineering); simulation training room as chaos platform — inject actual degraded equipment state into simulator; decay algorithm; HOP Learning Teams + chaos verification as a combined improvement loop. Ch. 19 (Database Chaos, Tang/Weng): TiDB's chaos practice — snapshot corruption motivating failure (page cache SLUB bug only found in production context); 5-step methodology (Prometheus steady state, hypothesis list, injection, verification, Schrodinger automation); fault taxonomy (application/CPU-memory/network/filesystem); Schrodinger platform (K8s-based, 7 concurrent clusters, 24/7 automated); network partition types (complete/partial/simplex); FUSE filesystem fault injection. Updated sources/chaos-engineering.

## [2026-05-28] ingest | Chaos Engineering ch. 16–17 (Rosenthal & Jones)

Ch. 16 (Continuous Verification, Rosenthal): CV as CI/CD/CV progression — proactive experimentation verifying output against expectations; verification over validation for complex systems; CV spectrum (automated canaries → chaos platforms → visualisation tools); ChAP architecture (Monocle prioritisation, control/variable instances, 45-minute experiments, SPS KPI, immediate abort on deviation); future CV directions (performance, data artifacts, correctness layers). Ch. 17 (Cyber-Physical, Aschbacher): CPSs — embedded hardware-software systems with life-critical consequences; FMEA practice and its limitations (single-point failures only, imagined effects vs real); Chaos Engineering as FMEA validation and extension; multipoint failure problem in software-intensive systems; experiment target priority (start with most-trusted critical assumptions); timing constraints as first target in CPSs; probe effect problem and mitigations; long-term vision of software "material properties." Updated operations/chaos-engineering with Continuous Verification section. Updated sources/chaos-engineering.

## [2026-05-28] ingest | Chaos Engineering ch. 14–15 (Rosenthal & Jones)

Ch. 14 (Open Chaos, Miles): adversarial vs collaborative mindset; co-ownership prerequisite; Open Science parallels; Open Chaos Initiative experiment schema (Contributions, Steady-State Hypothesis, Method, Rollbacks); double Steady-State Hypothesis assessment; reliability should not be a competitive differentiator. Ch. 15 (Chaos Maturity Model): two-axis model — Adoption (who bought in, organisational spread, prerequisites, obstacles) and Sophistication (Game Days → consultation → self-service → experimentation platform → automation); experiment layer progression (infrastructure → application → business logic); antipattern of multiple instance-kill variations; "controlled risk swaps for uncontrolled risk." Updated operations/chaos-engineering with Chaos Maturity Model section. Updated sources/chaos-engineering.

## [2026-05-28] ingest | Chaos Engineering ch. 12–13 (Rosenthal & Jones)

Ch. 12 (Experiment Selection, Alvaro): combinatorial explosion problem in chaos experiment selection; random search limitations; expert-guided selection limitations (communicability of intuition); Lineage-Driven Fault Injection (LDFI) — automated selection using Boolean formulae over distributed traces, SAT solver for experiment choice, ILP for prioritisation; deployed at Netflix/Huawei/eBay. Ch. 13 (ROI): attribution difficulty — "no one tells the story of the incident that didn't happen"; Kirkpatrick Model (Reaction/Learning/Transfer/Results); ChAP ROI case study — "SPS saved" metric achieved Level 2; self-limiting ROI dynamic; Collateral ROI (scenario design, team formation). Updated operations/chaos-engineering with ROI and Business Justification section and Automated Experiment Selection section. Updated sources/chaos-engineering.

## [2026-05-28] ingest | Chaos Engineering ch. 10–11 (Rosenthal & Jones)

Ch. 10 (Humanistic Chaos, Fleener): applying Chaos Engineering principles to organisations as complex systems — work-as-imagined vs work-as-done gap ("dark debt"); weak signals in human systems; Cook's principles; three case studies (Game Day SPOF detection, engineering rotation programme, employee-driven cultural experiments); Westrum's typology as readiness sniff test; leadership as emergent property. Ch. 11 (People in the Loop, Allspaw): Substitution Myth (Hollnagel) — automation cannot substitute for human action without changing the system; Ironies of Automation (Bainbridge) — automation creates new human tasks; function allocation/HABA-MABA critique; confidence-building as generative dialogue; context-sensitive timing of experiments; humans cannot be "out of the loop" (Winograd and Flores). Updated operations/chaos-engineering with expanded Disaster Simulations (Human Side) section and new Automation and Human Judgement section. Updated sources/chaos-engineering.

## [2026-05-28] ingest | Chaos Engineering ch. 08–09 (Rosenthal & Jones)

Ch. 8 (Capital One, Chockaiyan): chaos in regulated financial services — Blind Resiliency Testing since 2013; audit trail and compliance as core requirements; in-house tooling over third-party; chaos as CI/CD gate; co-creation team model; ROI via support call volume reduction; community of practice as evangelism mechanism. Ch. 9 (Creating Foresight, Nora Jones): Game Days as cognitive exercises for expertise transfer, not bug hunting; Law of Fluency (Woods — experts cannot perceive own expertise); cognitive interviews from aviation/medicine/maritime; resilience as positive adaptive capacity (Dekker); five Game Day roles; Before/During/After phases; ChAP Netflix experience; automation ironies; Gary Klein on expert vs novice mental models. Updated operations/chaos-engineering with Game Days as Cognitive Exercises and Regulated Environments sections. Updated sources/chaos-engineering.

## [2026-05-28] ingest | Chaos Engineering ch. 06–07 (Rosenthal & Jones)

Ch. 6 (Microsoft, Surmachev): experiment prioritisation — frequency/tolerance/imminence framework; three outcome categories (known/expected, known/unexpected, unknown/unexpected); variation spectrum (isolated/combined/compounded); "we get good at what we do often." Ch. 7 (LinkedIn, Rosen): blast radius minimisation via LinkedOut (request-level fault injection in Rest.li filter chain); three failure modes (error/delay/timeout); targeting progression (browser→employees→members); big red button requirement; Hawaii missile alert as UI design lesson. Updated operations/chaos-engineering with Experiment Prioritisation section, blast radius progression, big red button note. Updated sources/chaos-engineering.

## [2026-05-28] ingest | Chaos Engineering ch. 04–05 (Rosenthal & Jones)

Ch. 4 (Slack Disasterpiece Theater, Richard Crowley): structured Game Day process for legacy systems — prerequisites (spare capacity, automated replacement, correct timeouts), 3-phase process (preparation/exercise/debrief), failure technique progression (process→instance→iptables→partition), anti-goals (no exploratory chaos, no planned human intervention). Ch. 5 (Google DiRT, Jason Cahoon): DiRT programme since 2006 — five rules of engagement, test categories (SLO-level tests, soft-dependency removal, people outages, release/rollback, incident management, data integrity, capacity), Borg Eviction SLO case study (safe-listing as diagnostic), Hyrum's Law, Woods's Theorem. Updated operations/chaos-engineering with Game Days section, prerequisites, Disasterpiece Theater process, DiRT rules, Hyrum's Law. Updated sources/chaos-engineering.

## [2026-05-28] ingest | Chaos Engineering ch. 02–03 (Rosenthal & Jones)

Ch. 2: Dynamic Safety Model (Rasmussen) — engineers lack safety intuition, drift toward Economics/Workload poles away from Safety; chaos builds safety intuition. Economic Pillars of Complexity (Beck/Zaninotto) — State/Relationships/Environment/Reversibility; software's unique advantage is Reversibility; chaos surfaces brittleness as Reversibility targets. Ch. 3: Principles formalised — experimentation vs testing distinction; verification vs validation; "not breaking stuff"; antifragility distinguished from chaos engineering; five advanced principles including latency injection as the most informative experiment type. Updated operations/chaos-engineering with Dynamic Safety Model section, Economic Pillars section, Experimentation vs Testing section, and expanded Five Advanced Principles. Updated sources/chaos-engineering.

## [2026-05-28] ingest | Chaos Engineering ch. 00–01 (Rosenthal & Jones)

Created source page `wiki/sources/chaos-engineering.md`. Ingested preamble (Netflix origin story: Chaos Monkey, Chaos Kong, Principles of Chaos Engineering, community history) and ch. 1 (complex vs simple systems; accidental vs essential complexity; three emergent failure case studies: business-logic/app-logic mismatch, customer-induced retry storm, holiday code freeze). Updated `operations/chaos-engineering` with complexity foundations, origins section, and new source citation. Created author pages for Casey Rosenthal and Nora Jones. Updated index.md.
