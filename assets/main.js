...
(() => {
  ...
  if (window.PagefindUI) {
    const styleEl = document.querySelector('link[href*="assets/style"]');
    ...
    const searchDlg = document.querySelector("#search-dialog");
    const openSearch = () => {
      searchDlg?.showModal();
      setTimeout(() => searchDlg?.querySelector("input")?.focus(), 50);
    };
    searchToggle?.addEventListener("click", openSearch);
    searchDlg?.querySelector(".search-close")?.addEventListener("click", () => searchDlg.close());
    searchDlg?.addEventListener("click", (e) => {
      if (e.target === searchDlg) searchDlg.close();
    });
  ...
  const listenStop = document.querySelector(".listen-stop");
  if (listenBtn) {
    if (!("speechSynthesis" in window)) {
      const bar = listenBtn.closest(".listen-bar");
